<template>
  ...
  <div
    v-else
    class="container page-loaded-container pt-3"
  ...
  >
    <div class="row">
      ...
      <div
        v-if="person !== null && deleted === false"
      ...
      >
        ...
        <Modal
          ...
          :body="
            ...
            ' ' +
            person.familyName +
            ' is about to be archived. This operation cannot be undone!'
          "
        >
        </Modal>
      </div>
    ...
    <div v-if="person !== null" :class="{ 'text-muted': deleted }">
      ...
      <ul v-if="person.memberships.length > 0" class="list-group">
        <li
          v-for="group in memberships.values()"
          :key="group[0].groupId"
          class="list-group-item"
        >
          {{ group[0].groupName }}
          <span
            v-for="membership in group"
            :key="membership.id"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function onResponse(response: PersonResponse2) {
      ...
      document.title =
        response.givenName + " " + response.familyName + " - Korga";
    ...
    function createTimeString(
      prepend: string,
      actionTime: Date | null | undefined,
      author: PersonResponse | null | undefined
    ) {
      if (actionTime != null) {
    ...
    const creation = computed(() =>
      createTimeString(
        "Created on ",
        person.value?.creationTime,
        person.value?.createdBy
      )
    );
    const deletion = computed(() =>
      createTimeString(
        "Deleted on ",
        person.value?.deletionTime,
        person.value?.deletedBy
      )
    );
    ...
    const archive = function () {
      ...
      if (id === -1) {
        console.warn(
          "It should not be possible to archive a person if it is already archived."
        );
      } else {
        state.value.loaded = false;
